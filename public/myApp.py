...
window = tk.Tk()
window.geometry('850x655+350+150')
window.title('Employee Administration System')
window.configure(background='#6A6A6A')
window.resizable(width=False, height=False)

icon = tk.PhotoImage(file='emp_img.png')
...
def insertData():
	cur = conn.cursor()
	employee_id = emp_id.get(); employee_name = emp_name.get()
	employee_age = emp_age.get(); employee_email = emp_email.get()
	employee_phone = emp_phone.get(); employee_salary = emp_salary.get()
	date_emp = str(datetime.now().date())

	if employee_id == '' or employee_name == '' or employee_age == '' or employee_email == '' or employee_phone == '' or employee_salary == '':
		...
		val = (employee_id,employee_name,employee_age,employee_email,employee_phone,employee_salary,date_emp)
		cur.execute(insertQuery,val)

		if insertData:
			messagebox.showinfo("Employee Insertion","Employee has been inserted successfully")
...
def delete_employee():
	...
	if not employee_id in list_ids:
		messagebox.showinfo("Error","Employee does not exist")

	else:
		delMessage = messagebox.askquestion("Delete Employee","Are you sure that you want to delete this employee ?")
		if delMessage == 'yes':
			deleteQuery = ("DELETE FROM emp_data WHERE ID = ?")

			deleteEmployee = cur.execute(deleteQuery, (employee_id,))
			messagebox.showinfo("Delete Employee","Employee has been deleted succesfully")
			emp_id.set('');emp_name.set('');emp_age.set('');emp_email.set('');emp_phone.set('');emp_salary.set('')
			get_data()
		conn.commit()
...
def light_theme():
	...
	employee_data.config(bg='#C9FFF8',fg='#2B5667');employee_list.config(bg='#F4FFFF',fg='#31766D')
...
def dark_theme():
	...
	employee_data.config(bg='#6A6A6A',fg='#A4FFFB');employee_list.config(bg='#6A6A6A',fg='#E7FFD8')
...
my_app_title = tk.Label(App_title_frm,text='ıllıllı\tEmployee Management System\tıllıllı',bg='#4F5A60',
...
search_lbl = tk.Label(search_frm,text='Enter th ID or the name you are searching for',bg='#53595D',
...
search_btn = tk.Button(search_frm,width=12,padx=2,pady=2,
	font=("Arial Greek",10),bd=1,text="☥ Search",bg="#56545D",fg='white',relief=tk.RIDGE,command=search_employee)
...
employee_data = tk.LabelFrame(window,width=830,height=150,bd=3,font=('Arial Greek',10,'bold'),
	text='Employee Information',bg='#6A6A6A',fg='#A4FFFB')
buttons_box = tk.LabelFrame(window,width=830,height=80,bd=3,font=('Arial Greek',10,'bold'),
	bg='#5C6463')
employee_list = tk.LabelFrame(window,width=830,height=150,bd=3,font=('Arial Greek',10,'bold'),
...
def update_employee():
	...
	values_update = (emp_name.get(),emp_age.get(),emp_email.get(),emp_phone.get(),emp_salary.get(),
		str(cal1.get_date()),employee_id)
	cur.execute(update_query,values_update)

	if employee_id in emp_ids_list:
		messagebox.showinfo("Employee Update","Employee has been updated successfully")
		emp_id.set('');emp_name.set('');emp_age.set('');emp_email.set('');emp_phone.set('');emp_salary.set('')

	else:
		messagebox.showinfo("ID does not exist","This employee does not exist in the database")
	conn.commit()
	get_data()

# ========================= Employee Information start =========================
ent_color = '#56666B'
idlbl = tk.Label(employee_data,text='Employee ID',font=('Arial Greek',10,'bold'),bg='#6A6A6A',
	fg='white')
...
namelbl = tk.Label(employee_data,text='Employee name',font=('Arial Greek',10,'bold'),bg='#6A6A6A',
	fg='white')
...
agelbl = tk.Label(employee_data,text='Employee age',font=('Arial Greek',10,'bold'),bg='#6A6A6A',
	fg='white')
...
emaillbl = tk.Label(employee_data,text='Employee email',font=('Arial Greek',10,'bold'),bg='#6A6A6A',
	fg='white')
...
phonelbl = tk.Label(employee_data,text='Employee phone',font=('Arial Greek',10,'bold'),bg='#6A6A6A',
	fg='white')
...
salarylbl = tk.Label(employee_data,text='Employee salary',font=('Arial Greek',10,'bold'),bg='#6A6A6A',
	fg='white')
salarylbl.place(x=400,y=80)

salaryEnt = tk.Entry(employee_data,width=17,textvariable=emp_salary,font=('Arial Greek',12),
	bg=ent_color,fg='#131414',relief=tk.RIDGE)
salaryEnt.place(x=520,y=80)

# ============================ Employee Information End =======================
trView = ttk.Treeview(employee_list,columns=(1,2,3,4,5,6),show="headings",height=8)
trView.pack(fill='both')

trView.heading(1,text="ID"); trView.heading(2,text="Name")
trView.heading(3,text="Age");trView.heading(4,text="Email")
trView.heading(5,text="Phone");trView.heading(6,text="Salary")
...
updateEmployee = tk.Button(buttons_box,width=12,padx=2,pady=2,
	font=("Arial Greek",12),bd=1,text="© Update",bg="#435C6B",fg='white',relief=tk.RIDGE,command=update_employee)
updateEmployee.place(x=355,y=20)

retrieveEmployee = tk.Button(buttons_box,width=12,padx=2,pady=2,
	font=("Arial Greek",12),bd=1,text="ㆁ Retrieve",bg="#476265",fg='white',relief=tk.RIDGE,command=retrive_employee)
retrieveEmployee.place(x=195,y=20)

deleteEmployee = tk.Button(buttons_box,width=12,padx=2,pady=2,
	font=("Arial Greek",12),bd=1,text="〤 Delete",bg="#6C6354",fg='white',relief=tk.RIDGE,command=delete_employee)
